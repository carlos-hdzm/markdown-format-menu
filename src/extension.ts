...
import {
  ...
  link,
  image,
} from "./factories/inline-wrappers.js";
...
import {
  ...
  heading5,
  heading6,
} from "./factories/block-prependers.js";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let unorderedListBullet =
    vscode.workspace
      .getConfiguration("markdown-format-menu")
      .get<string>("unorderedListBulletString") || "-";
  let unorderedListCommand = vscode.commands.registerCommand(
    "markdown-format-menu.unordered-list",
    unorderedList(unorderedListBullet),
  );

  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("markdown-format-menu")) {
        unorderedListCommand.dispose();

        unorderedListBullet =
          vscode.workspace
            .getConfiguration("markdown-format-menu")
            .get<string>("unorderedListBulletString") || "-";
        unorderedListCommand = vscode.commands.registerCommand(
          "markdown-format-menu.unordered-list",
          unorderedList(unorderedListBullet),
        );
      }
